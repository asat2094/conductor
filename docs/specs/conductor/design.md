...
**New:** `decompose.py`, `dag.py`, `lint_plan.py`, `contracts.json` (+ schema), `characterize.sh` + `golden/`, `workspace.py`, `merge_queue.py`, `admission.py`, `retrieve.py`, `baseline.json`, `prompts/spec_auditor.txt`, `heartbeat.py` (stall-only v1), `compress.py`, `orchestrator_gate.py` (REQ-O2/O3), `codegraph_adapter.py` (codegraphcontext MCP + fallback).
