...
**New (reliability gates + loop, ADR-0025/0026/0027):** property-based + metamorphic tier in `evaluator.py` (Hypothesis, pinned seed — first-class for functional units, REQ-T10/T11); **held-out acceptance oracle** path (spec/human-derived, impl-maker never sees it; mandatory high-stakes — REQ-T12); **anti-reward-hacking scope guard** in the scope check (test-file/`conftest.py`/`sys.exit`/`__eq__`/deleted-assertion patterns → scope violation, REQ-T13); **dependency-existence check** (`harness/deps_check.py` — PyPI resolve before install, REQ-A4); **bounded repair loop** in `healer.py` (mechanical-feedback generate→gate→repair, stuck-detection, hard caps — REQ-H1/H2/H3). Blind-spot defense narrowed to a **single independent validator + mechanical gates** (ADR-0004 amendment) — no ensembles/debate.
