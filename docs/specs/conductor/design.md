...
- **Decomposition verifier residual (ADR-0022):** `verify.py` is bounded by static-analysis accuracy. Dynamic dispatch, reflection, monkeypatching, and string-based imports produce coupling codegraph cannot see → a wrong-grouping can still pass the verifier. Recorded deliberately to keep trust calibrated: the verifier *reduces* L1/L3/L4, it does not *close* them; the assembly golden gate (ADR-0004) remains the empirical backstop. Also new tuning debt: the density/decomposability threshold (REQ-D8) and the high-confidence cutoff for under-declared-edge ERRORs.
