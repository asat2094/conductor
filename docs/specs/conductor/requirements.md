...
- **REQ-T10** *(property-based gate, ADR-0025)* — AFTER GREEN, THE SYSTEM SHALL run property-based tests (Hypothesis, pinned seed) over each changed function's declared invariants; a surviving counterexample SHALL be a hard gate failure feeding the repair loop, and the minimized counterexample SHALL be recorded for replay. Properties SHALL be contract/spec-derived, never impl-maker-authored.
- **REQ-T11** *(metamorphic relations, ADR-0025)* — WHERE a metamorphic relation applies (round-trip, ordering, scale, idempotency), THE SYSTEM SHALL assert it mechanically as part of the property-based tier. Units with no applicable relation fall back to PBT-over-invariants + example tests.
- **REQ-T12** *(held-out acceptance oracle, ADR-0026)* — THE SYSTEM SHALL keep a spec/orchestrator-derived (or human-provided) **held-out acceptance test** that the impl-maker's context/worktree never receives; a unit SHALL be ACCEPTED only when both the in-loop GREEN and the held-out oracle pass. The held-out oracle is **mandatory for high-stakes units** (NFR-SEC-1), optional otherwise. It SHALL NOT be authored by the impl-maker.
- **REQ-T13** *(anti-reward-hacking scope guard, ADR-0026)* — Before acceptance, THE SYSTEM SHALL mechanically scan the unit diff and reject (scope violation, not advisory) any edit to a test file / `conftest.py`, introduction of `sys.exit(`, `__eq__`/`__ne__` overrides on test-touched types, or deleted/weakened assertions — except for declared test-authoring task-types.
...
- **REQ-A4** *(dependency-existence check, ADR-0026)* — Before any maker-proposed dependency install / new third-party import, THE SYSTEM SHALL verify the package name resolves on the live registry (PyPI); an unresolvable name SHALL be rejected (slopsquatting defense). Offline → flag `unverified-dependency`, do not auto-install.

### Heal / repair loop (ADR-0027)
- **REQ-H1** *(mechanical stop only)* — The repair loop's stop condition SHALL be mechanical (gate pass) or escalation; it SHALL NOT add a model-judges-good-enough soft gate to break a stall. A fuzzy goal not expressible as a passing gate SHALL escalate to a human/frontier model.
- **REQ-H2** *(stuck-detection)* — THE SYSTEM SHALL abort the repair loop and escalate WHEN the gate's failure signal is byte-identical (or the produced diff is empty) across the last N attempts (default 2), rather than burning the remaining budget.
- **REQ-H3** *(hard caps)* — The repair loop SHALL enforce a per-unit attempt ceiling (default 3, hard max < 10) AND a per-unit token/cost ceiling tied to the cost model / admission ceiling; reaching either SHALL escalate. Failure feedback to the maker SHALL be the mechanical gate output only (failing test, PBT counterexample, mutation survivor, scope reason).
